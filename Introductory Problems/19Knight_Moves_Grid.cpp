...
vector<ll>delRow1= {2,2,1,-1,1,-1,-2,-2};
vector<ll>delCol1 = {-1,1,2,2,-2,-2,1,-1};
bool validate(ll i, ll j, ll n){
    return i>=0 && j>=0 && i<n && j<n;
}
void solve(vector<vector<ll>>&grid, ll n){
    queue<pair<ll,ll>>q;
    grid[0][0]=0;
    q.push({0,0});
    while(!q.empty()){
        ...
        q.pop();
        for(ll k=0;k<8;k++){
            ll newRow = i+delRow1[k];
            ll newCol = j+delCol1[k];
            if(validate(newRow,newCol,n)){
