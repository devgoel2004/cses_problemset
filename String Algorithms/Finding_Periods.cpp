...
ll nearestLowerPowerOfk(ll n, ll k){
    if(n < 1){
        return 0;
    }
    double logValue = log(n) / log(k);
    ll exponent = floor(logValue);
...
vector<ll>create_pref(vector<ll>&a){
    ll n = a.size();
...
void hareKrishna(){   
    ...
    auto get_hash = [&](ll l, ll r)->ll{
        ll h = (pref[r+1]
            - (pref[l] * base_pow[r-l+1])%m
            + m) % m;
        return h;
    };
    for(ll i=0;i<n;i++){
        ...
        bool check = true;
        while(curr < n){
            ll len = min(i+1,n-curr);
            check &= get_hash(0,len-1) == get_hash(curr, curr+len-1);
            curr += len;
        }
        if(check){
            cout<<i+1<<' ';
