...
void inverseFactorial(){
    invfact[1e6] = power(fact[1e6],MOD-2);
    for(ll i=1e6;i>=1;i--){
...
ll find_no_of_factors(vector<pair<ll,ll>>&vp){
    ll res = 1;
    for(auto x:vp){
        res = (res%MOD* (x.second%MOD + 1%MOD)%MOD)%MOD;
    }
    return res;
}
ll geometric_mean(ll x, ll y){
    ll pw = (power(x,y+1)%MOD - 1%MOD + MOD)%MOD;
    ll val = power((x%MOD-1%MOD+MOD)%MOD, MOD-2);
...
void hareKrishna(){
    ...
    sort(vp.begin(),vp.end(),[&](pair<pair<ll,ll>,ll>&a, pair<pair<ll,ll>,ll>&b){
        return a.first.first < b.first.first;
    });
    
    vector<ll>ans(n);
    multiset<pair<ll,ll>>st;
    st.insert({vp[0].first.second,1});
    ans[vp[0].second-1]= 1;
    for(ll i=1;i<n;i++){
        // case 1 
        ll st_val = vp[i].first.first;
        ll end = vp[i].first.second;
        ll ind = vp[i].second;
        auto [smallest, rid] = *begin(st);
        if(smallest < st_val){
            st.erase(st.begin());
            ans[ind-1] = rid;
            st.insert({end,rid});
