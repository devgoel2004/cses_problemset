...
void hareKrishna(){
    ...
    ll no_of_factors = find_no_of_factors(vp);
    cout<<no_of_factors<<' ';
    ll sum = 1;
    for(auto x:vp){
        ll val = geometric_mean(x.first,x.second);
    ...
    ll exponent = 1;
    for(auto [p, k] : vp){
        ll term1 = (k * (k + 1) / 2) % (MOD - 1);
        
        product = power(product, k + 1) % MOD;
        product = (product * power(p, term1 * exponent % (MOD - 1))) % MOD;
        
        exponent = exponent * (k + 1) % (MOD - 1);
    }
    cout<<product<<'\n';
    // cout<<sum<<'\n';
}
