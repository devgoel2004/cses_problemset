...
ll N = 1e6+1;
vector<ll>smallest_prime_factors(N+1,1e9);
vector<bool> sieveOfEratosthenes(ll N) {
    vector<bool> isPrime(N + 1, true);
    isPrime[0] = isPrime[1] = false;
    for (ll p = 2; p * p <= N; ++p) {
        if (isPrime[p]) {
            smallest_prime_factors[p] = p;
            for (ll i = p * p; i <= N; i += p) {
                isPrime[i] = false;
                smallest_prime_factors[i] = min(smallest_prime_factors[i],p);
            }
        }
    }
    return isPrime;
...
void hareKrishna() {
    ...
    ll ans=1;
    while(n!=1){
        ll smallest = smallest_prime_factors[n];
        ll cnt=0;
        while(n%smallest==0){
            n/=smallest;
            cnt++;
        }
        ll power = (solve(smallest, cnt+1)%MOD - 1%MOD + MOD)%MOD;
        ll val = (solve(smallest-1,MOD-2))%MOD;
        ll term = (power%MOD * val%MOD)%MOD;
